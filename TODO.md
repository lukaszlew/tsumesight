...
## 1 - Better button layout for mobile
...
## 1 - Cap question time at 5s for scoring
...
## 1 - Make ghost stone opaque with move number
...
## 2 - Ask about groups with fewer liberties first
...
## 2 - Mode selection per problem instead of persistent config
Replace the liberty/comparison mode toggle in the gear menu with a mode
choice shown before each problem starts (two buttons: "Liberty"
"Comparison"). Remove mode from settings. Choice applies to current problem only.
...
## 2 - Show all stones with move numbers on mistake
...
## 2 - Skip unchanged comparison pairs
...
## 2 - Visual error feedback (non-blocking)
