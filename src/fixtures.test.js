// Layer A — fold fixture events through the session reducer, snapshot
// derived view at each step, cross-check goldens. This is the primary
// regression safety net for the V4 refactor.
//
// For each fixture under fixtures/:
//   1. Load {sgf, config, events, goldens}
//   2. init(sgf, config)
//   3. Apply events one by one via step(), snapshotting state between each
...
import {
  init, step, phase, finalized, changedGroups, mistakesByGroup, totalMistakes,
} from './session.js'
...
function snapshotState(s) {
  ...
  return {
    ...
    mistakesByGroup: (finalized(s) || phase(s) === 'finished') ? mistakesByGroup(s) : null,
  }
}

const fixtures = loadFixtures()

describe('Layer A — fold fixture events through the session reducer', () => {
  ...
  for (let { name, data } of fixtures) {
    it(`${name}: fold events and snapshot derived view`, async () => {
      let s = init(data.sgf.content, data.config)
    ...
    it(`${name}: goldens cross-check`, () => {
      let s = init(data.sgf.content, data.config)
      for (let evt of data.events) step(s, evt)
      let g = data.goldens || {}

      if (g.scoreEntry) {
        expect(totalMistakes(s), 'total mistakes').toBe(g.scoreEntry.mistakes)
        expect(changedGroups(s).length, 'changed groups count').toBe(g.scoreEntry.total)
        expect(mistakesByGroup(s), 'mistakes per group').toEqual(g.scoreEntry.mistakesByGroup)
      }
      if (g.finalMarks) {
      ...
      if (g.changedGroupsVertices) {
        expect(changedGroups(s).map(cg => cg.vertex)).toEqual(g.changedGroupsVertices)
      }
    })
  }
})
